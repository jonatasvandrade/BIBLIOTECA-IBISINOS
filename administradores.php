...
<?php
	/*@$filtro_nome = $_SESSION["div.filtro.administradores.nome"];
	@$filtro_usuario = $_SESSION["div.filtro.administradores.usuario"];
	@$filtro_email = $_SESSION["div.filtro.administradores.email"];
	@$paginacao = $_SESSION["div.filtro.administradores.paginacao"];
	if(trim($paginacao) == ""){
		$paginacao = 1;	
	}*/
...
<a href="administradores-form.php"><button type="button" class="btn btn" style="float:right;margin-top:17px;margin-right:5px;"><i class="icon-plus icon-dark"></i>&nbsp;Adicionar</button></a>
...
$("#conteudo_grid").load('administradores_ajax.php', function(response, status, xhr) {
	if (status == "success")
	{ }
});
...
//EXECUTA A PAGINAÇÃO
paginacao('administradores_ajax.php','<?php echo $paginacao; ?>');
...
function pesquisar(){
	...
	paginacao('administradores_ajax.php?'+url,pagina_atual);
