...
CriaConexao();
	...
	/*@$filtro_nome = $_SESSION["div.filtro.membros.nome"];
	@$filtro_usuario = $_SESSION["div.filtro.membros.usuario"];
	@$filtro_email = $_SESSION["div.filtro.membros.email"];
	@$paginacao = $_SESSION["div.filtro.membros.paginacao"];
	if(trim($paginacao) == ""){
		$paginacao = 1;	
	}*/
...
<a href="membros-form.php"><button type="button" class="btn btn" style="float:right;margin-top:17px;margin-right:5px;"><i class="icon-plus icon-dark"></i>&nbsp;Adicionar</button></a>
...
$("#conteudo_grid").load('membros_ajax.php', function(response, status, xhr) {
	if (status == "success")
	{ }
});
...
//EXECUTA A PAGINAÇÃO
paginacao('membros_ajax.php','<?php echo $paginacao; ?>');
...
function pesquisar(){
	...
	paginacao('membros_ajax.php?'+url,pagina_atual);
